...
# Warnings
CFLAGS += -fdiagnostics-color=always -Wall -Werror -Wextra -Wcast-align -Wconversion -Wfloat-equal -Wformat=2 -Wnull-dereference -Wshadow -Wsign-conversion -Wswitch-default -Wswitch-enum -Wundef -Wunreachable-code -Wunused -g -fsanitize=thread
...
.PHONY: all clean depend fmt first_test first_clean
...
first_clean:
	make -f Makefile_deprecated clean

first_test:
	make -f Makefile_deprecated test
